...
export const SvgSelector = (props) => {
  const svgMap = {
    messenger: (
      <svg className={props.className} viewBox="0 0 200 200">
        <path
    ...
    settings: (
      <svg strokeWidth="0.1" className={props.className} viewBox="0 0 24 24">
    ...
    comment: (
      <svg className={props.className} viewBox="0 0 211 200">
        <path
  ...
  if (!svgMap.hasOwnProperty(props.id)) {
    console.warn(`Svg with id ${props.id} doesn't exist`);
    return svgMap.placeholder;
  }
  return svgMap[props.id];
};
